    ...
    const bookingResult = await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          message: `Slot booked successfully for ${cropType} (${quantity} Q). Token: ${tokenNumber} at ${slot.centre.name} on ${slot.date} (${slot.startTime}-${slot.endTime}).`,
        },
      });

      return { booking, procurement, slot };
    });

    // Simulated SMS alert to farmer
    logSimulatedSMS(
      farmer.mobileNumber,
      `Kisan Setu: Slot confirmed for ${cropType} (${quantity} Quintals). Token: ${bookingResult.booking.tokenNumber}. Date: ${bookingResult.slot.date}, Time: ${bookingResult.slot.startTime}-${bookingResult.slot.endTime} at ${bookingResult.slot.centre.name}. Ref: ${bookingResult.booking.bookingReference}. Please arrive 15 mins early.`
    );
